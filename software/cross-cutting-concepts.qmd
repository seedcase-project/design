...
-   The **OAuth** approach is more complicated compared to previously
    two. Here are some basic steps.

    1.  User requests access to a Data Resource managed by the Seedcase
        framework.
    2.  The Seedcase framework redirects the user to an OAuth server
    ...
    4.  The OAuth server sends the secret key back to the Seedcase
        framework.
    5.  The Seedcase framework uses the secret key from OAuth server,
        providing correct authorization of the user for accessing
        components of the Data Resource.
...
authentication as described above, as well as managing [user
permission](#permissions) levels.
...
be possible for that user to set the framework up in a way that they
...
project that is using the Seedcase framework to handle their data.

The Seedcase framework will ship with the following user roles
pre-defined to make it easier for the data project administrator to
ensure the security and privacy of the data entered into the system.
...
permissions to specific tasks is listed in the [User
permissions](#permissions) subsection below.
...
User roles and use cases are described in more detail on the [User Requirements](user-requirements.qmd) page.
