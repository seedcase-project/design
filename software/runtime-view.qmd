...
![Login and authentication sequence of a registered
user.](/design/images/runtime-login-sequence.png){#fig-login-sequence}
...
-   *Path*: The computational and programmatic steps the input data and
    ...
    automatically by a program) into a Seedcase Data Resource, passing
    through quality control checks, adding to the changelog, and storage
    into the database.
...
a Seedcase Data Resource. Health data tends to consist of specific types
of data:
...
-   *Routine or continuous collection*, where data input into a Seedcase
    Data Resource would occur as soon as the data was collected from one
    ...
    category.
-   *Batch collection*, where data ingestion occurs some time after the
...
redirecting these data sources from their generation into a Seedcase
Data Resource via a direct call to the API so the data continues on to
the backend and eventual data storage.
...
completes all fields in the form and clicks "Save and Submit". This
sends the data to the API layer where it is confirmed as valid,
...
data to the database along the incomplete flag.
...
request.](/design/images/runtime-data-request.png){#fig-data-request}
...
User](user-requirements.qmd#roles-and-use-cases) might take when browsing the Data Resource:
