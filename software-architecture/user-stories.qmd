...
|Data Request Admin|send an application back to the requester|so that they can fill out the missing values|
|Data Request Admin|comment on a data request|so that I can inform either the requester or the data controller of my findings|
...
|As a... |I want to... |Because... |
| --- | --- | --- |
|Data Requester|submit a request for data|so that I can continue my research|
|Data Requester|see how far my request is in the system|so that I know when I'm likely to hear of a decision|
|Data Requester|see the variables that the system contains|so that I can decide if I should apply for data|
|Data Requester|pick and chose between variables|so that I can put together a variable list for an application for data|
|Data Requester|ask questions about variables|so that I can get clarificaton before asking for data|
|Data Requester|select between different formats when I submit a data request|so that I get the data in a format I can work with|
...
|Data Contributor|define a field (or set of fields) as a primary key for a table|so that I can link data between tables|
|Data Contributor|define a field (or set of fields) as a foreign key for a table|so that I can link data between tables|
...
|Data Contributor|upload a file without a receiving table|so that I can use the system to fetch all the headings and data types, helping me to create a new table|
|Data Contributor|select from a sets of predefined tables|so that I don't have to type in all the definitions, if I'm uploading data from a particular machine or system|
